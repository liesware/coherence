...
////////////////////////////////////////////////////////////////////////
template <typename T>
int BLOCK_ENC_GCM(string& payload, string& result, string& key, string& iv ,int& binary, string& error){
  result.clear();
  error.clear();
  SecByteBlock key_b(key.size()/2),iv_b(iv.size()/2);
  string key_e,iv_e;
  ...
  try{
    ...
    StringSource( payload, true,
        new AuthenticatedEncryptionFilter( block,
            new StringSink( result_e ), false, TAG_SIZE
        )
    );
...
template <typename T>
int BLOCK_DEC_GCM(string& payload, string& result, string& key, string& iv ,int& binary, string& error){
  result.clear();
  error.clear();
  SecByteBlock key_b(key.size()/2),iv_b(iv.size()/2);
  string key_e,iv_e;
  ...
  try{
    ...
    string payload_b,result_d;
    StringSource(payload, true,new HexDecoder(new StringSink(payload_b)));

    StringSource( payload_b, true, new AuthenticatedDecryptionFilter(block,
        new StringSink( result_d ),
    ...
    if(Isjson(result,error)!=0){
        result.clear();
        error.clear();
        StringSource(result_d, true,new HexEncoder(new StringSink(result)));
      }
    }
    catch(const CryptoPP::Exception& e){
      error=e.what();
  ...
  if(d.HasMember("type") ){
    ...
    if(strncmp(req_val.operation.c_str(), "enc",sizeof("enc")) == 0){
      if(strncmp(req_val.mode.c_str(), "ctr",sizeof("ctr")) == 0){
        ...
        if(strncmp(req_val.algorithm.c_str(), "AES",sizeof("AES")) == 0){
          BLOCK_ENC_GCM<GCM< AES >::Encryption>(req_val.payload, req_val.result, req_val.key , req_val.iv, req_val.hex, req_val.error);
        }
        else if(strncmp(req_val.algorithm.c_str(), "RC6",sizeof("RC6")) == 0){
          BLOCK_ENC_GCM<GCM< RC6 >::Encryption>(req_val.payload, req_val.result, req_val.key , req_val.iv, req_val.hex, req_val.error);
        }
        else if(strncmp(req_val.algorithm.c_str(), "MARS",sizeof("MARS")) == 0){
          BLOCK_ENC_GCM<GCM< MARS >::Encryption>(req_val.payload, req_val.result, req_val.key , req_val.iv, req_val.hex, req_val.error);
        }
        else if(strncmp(req_val.algorithm.c_str(), "SERPENT",sizeof("SERPENT")) == 0){
          BLOCK_ENC_GCM<GCM< Serpent >::Encryption>(req_val.payload, req_val.result, req_val.key , req_val.iv, req_val.hex, req_val.error);
        }
        else if(strncmp(req_val.algorithm.c_str(), "TWOFISH",sizeof("TWOFISH")) == 0){
          BLOCK_ENC_GCM<GCM< Twofish >::Encryption>(req_val.payload, req_val.result, req_val.key , req_val.iv, req_val.hex, req_val.error);
        }
        else if(strncmp(req_val.algorithm.c_str(), "CAST256",sizeof("CAST256")) == 0){
          BLOCK_ENC_GCM<GCM< CAST256 >::Encryption>(req_val.payload, req_val.result, req_val.key , req_val.iv, req_val.hex, req_val.error);
        }
        else if(strncmp(req_val.algorithm.c_str(), "CAMELLIA",sizeof("CAMELLIA")) == 0){
          BLOCK_ENC_GCM<GCM< Camellia >::Encryption>(req_val.payload, req_val.result, req_val.key , req_val.iv, req_val.hex, req_val.error);
        }
        else if(strncmp(req_val.algorithm.c_str(), "SPECK128",sizeof("SPECK128")) == 0){
          BLOCK_ENC_GCM<GCM< SPECK128 >::Encryption>(req_val.payload, req_val.result, req_val.key , req_val.iv, req_val.hex, req_val.error);
        }
        else{
          req_val.error="Bad Block algorithm ";
          answ_error(req_val,answ_js);
      ...
      if(strncmp(req_val.mode.c_str(), "ctr",sizeof("ctr")) == 0){
        ...
        if(strncmp(req_val.algorithm.c_str(), "AES",sizeof("AES")) == 0){
          BLOCK_DEC_GCM<GCM< AES >::Decryption>(req_val.payload, req_val.result, req_val.key , req_val.iv, req_val.hex, req_val.error);
        }
        else if(strncmp(req_val.algorithm.c_str(), "RC6",sizeof("RC6")) == 0){
          BLOCK_DEC_GCM<GCM< RC6 >::Decryption>(req_val.payload, req_val.result, req_val.key , req_val.iv, req_val.hex, req_val.error);
        }
        else if(strncmp(req_val.algorithm.c_str(), "MARS",sizeof("MARS")) == 0){
          BLOCK_DEC_GCM<GCM< MARS >::Decryption>(req_val.payload, req_val.result, req_val.key , req_val.iv, req_val.hex, req_val.error);
        }
        else if(strncmp(req_val.algorithm.c_str(), "SERPENT",sizeof("SERPENT")) == 0){
          BLOCK_DEC_GCM<GCM< Serpent >::Decryption>(req_val.payload, req_val.result, req_val.key , req_val.iv, req_val.hex, req_val.error);
        }
        else if(strncmp(req_val.algorithm.c_str(), "TWOFISH",sizeof("TWOFISH")) == 0){
          BLOCK_DEC_GCM<GCM< Twofish >::Decryption>(req_val.payload, req_val.result, req_val.key , req_val.iv, req_val.hex, req_val.error);
        }
        else if(strncmp(req_val.algorithm.c_str(), "CAST256",sizeof("CAST256")) == 0){
          BLOCK_DEC_GCM<GCM< CAST256 >::Decryption>(req_val.payload, req_val.result, req_val.key , req_val.iv, req_val.hex, req_val.error);
        }
        else if(strncmp(req_val.algorithm.c_str(), "CAMELLIA",sizeof("CAMELLIA")) == 0){
          BLOCK_DEC_GCM<GCM< Camellia >::Decryption>(req_val.payload, req_val.result, req_val.key , req_val.iv, req_val.hex, req_val.error);
        }
        else if(strncmp(req_val.algorithm.c_str(), "SPECK128",sizeof("SPECK128")) == 0){
          BLOCK_DEC_GCM<GCM< SPECK128 >::Decryption>(req_val.payload, req_val.result, req_val.key , req_val.iv, req_val.hex, req_val.error);
        }
        else{
          req_val.error="Bad Block algorithm ";
          answ_error(req_val,answ_js);
