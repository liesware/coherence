...
//Key/////////////////////////////////////////////////////////////////
int check_key(Document& d, stru_param& req_val, string& answ_js, int key_size=64){
  if(d.HasMember("key") && d["key"].IsString()){
    ...
    if(key_size==64){
      if(req_val.key.size()>64){
        req_val.error+=" Key size bad ";
        req_val.tag="error";
        Addstr2json(answ_js, req_val.tag, req_val.error);
        #ifdef DEBUG
        cerr << req_val.error;
...
//Iv/////////////////////////////////////////////////////////////////
int check_iv(Document& d, stru_param& req_val, string& answ_js, int vec_size=32){
  if(d.HasMember("iv") && d["iv"].IsString()){
    ...
    if(req_val.iv.size()!=vec_size){
      req_val.error+=" Iv size bad ";
      req_val.tag="error";
      Addstr2json(answ_js, req_val.tag, req_val.error);
      #ifdef DEBUG
      cerr << req_val.error;
